...
def indicator_cal(feat1, feat2):
    batchsize = feat1.shape[0]
    feat1 = feat1.permute(0, 2, 3, 1)
    feat2 = feat2.permute(0, 2, 3, 1)
    feat_dot = []
    for i in range(batchsize):
        tmp_feat1 = feat1[i].view(-1, feat1.shape[-1])
        tmp_feat2 = feat2[i].view(-1, feat2.shape[-1])
        feat_mul = tmp_feat1 * tmp_feat2
        feat_dot.append(feat_mul.sum(dim=1))
    feat_dot = torch.cat(feat_dot, dim=0)
    return feat_dot.min(), feat_dot.max(), feat_dot.mean()
